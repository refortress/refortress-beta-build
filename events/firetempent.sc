...
public Fire_Glock()
{
	...
	Native_PlayPlayerSound(dfg $player = $localplayer, $sound = $snd, $channel = CHAN_STATIC, $pitch = $def_pitch, $attenuation = $def_attn, $pitch = $def_pitch );

	// Eject shellsdfg
	//
	for ( $i = 0; $i < 2; $i++ )
	{
		$shell_origdfdin   = $origin + $view_ofs;
		...
		(
			...
			$te_baselinfdfseangles = Native_RandomAngles(),
